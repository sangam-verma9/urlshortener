...
const Home = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const result = await createUrl(longUrl.trim())

            if (result.success) {
                setShortUrl(`${domain}/r/${result.url.key}`)
                setOriginalUrl(result.url.value)
                setLongUrl('')
            } else {
                setError(result.message || 'Failed to shorten URL. Please try again.')
            }
        } catch (err) {
